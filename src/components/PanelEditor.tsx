...
export function PanelEditor({ isOpen, onClose }: PanelEditorProps) {
  const {
    selectedShapeId, shapes, updateShape, addShape, showOutlines, setShowOutlines,
    showRoleNumbers, setShowRoleNumbers, selectedPanelRow, setSelectedPanelRow,
    panelSelectMode, setPanelSelectMode, raycastMode, setRaycastMode,
    showVirtualFaces, setShowVirtualFaces, virtualFaces, updateVirtualFace,
    deleteVirtualFace, pendingPanelCreation
  } = useAppStore();
  ...
  const getPanelDimensions = (faceIndex: number): { primary: number; secondary: number; thickness: number; w: number; h: number; d: number } | null => {
    ...
    const panel = shapes.find(s =>
      s.type === 'panel' && s.parameters?.parentShapeId === selectedShape.id &&
      s.parameters?.faceIndex === faceIndex && !s.parameters?.extraRowId
    );
    ...
    const axes = [{ index: 0, value: sizeVec.x }, { index: 1, value: sizeVec.y }, { index: 2, value: sizeVec.z }];
    axes.sort((a, b) => a.value - b.value);
    ...
    const dimByAxis = (ax: number) => ax === 0 ? dimensions.w : ax === 1 ? dimensions.h : dimensions.d;
    return { primary: dimByAxis(targetAxis), secondary: dimByAxis(secondaryAxis), thickness: dimByAxis(thicknessAxis), ...dimensions };
  };
  ...
  useEffect(() => {
    ...
    if (selectedProfile !== 'none') {
      setResolving(true);
      resolveAllPanelJoints(selectedShapeId, selectedProfile).finally(() => setResolving(false));
    } else {
  ...
  useEffect(() => {
    ...
    const geometryKey = [
      selectedShape.parameters?.width, selectedShape.parameters?.height, selectedShape.parameters?.depth,
      selectedShape.geometry?.uuid, (selectedShape.subtractionGeometries || []).length,
      JSON.stringify(selectedShape.position), JSON.stringify(selectedShape.scale)
    ].join('|');
    ...
    selectedShape?.parameters?.width, selectedShape?.parameters?.height, selectedShape?.parameters?.depth,
    selectedShape?.geometry?.uuid, selectedShape?.subtractionGeometries?.length,
    selectedShape?.position, selectedShape?.scale, selectedShapeId, selectedProfile
  ]);
    ...
    (async () => {
      try {
        ...
        const newPanel: any = {
          ...
          color: '#ffffff',
          parameters: {
            width: 0, height: 0, depth: panelThickness,
            parentShapeId: currentShape.id, faceIndex: -(vfIdx + 1),
            faceRole: vf.role, virtualFaceId: vf.id,
          }
        };
        addShape(newPanel);
  ...
  const createPanelForFace = async (faceGroup: CoplanarFaceGroup, faces: FaceData[], faceIndex: number) => {
    if (!selectedShape || !selectedShape.replicadShape) return;
    try {
      ...
      const newPanel: any = {
        ...
        color: '#ffffff',
        parameters: { width: 0, height: 0, depth: panelThickness, parentShapeId: selectedShape.id, faceIndex, faceRole }
      };
      addShape(newPanel);
    } catch (error) {
  ...
  return (
    <div
      ...
      style={{ left: `${position.x}px`, top: `${position.y}px`, width: '565px' }}
    >
      <div
        ...
        {selectedShape ? (
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1">
                <label className="text-xs font-semibold text-slate-800 whitespace-nowrap">Raycast Face</label>
                <input type="checkbox" checked={showVirtualFaces} onChange={(e) => setShowVirtualFaces(e.target.checked)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
              </div>
              <div className="flex items-center gap-1">
                <label className="text-xs font-semibold text-slate-800 whitespace-nowrap">Outline</label>
                <input type="checkbox" checked={showOutlines} onChange={(e) => setShowOutlines(e.target.checked)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
              </div>
              <div className="flex items-center gap-1">
                <label className="text-xs font-semibold text-slate-800 whitespace-nowrap">Role numbers</label>
                <input type="checkbox" checked={showRoleNumbers} onChange={(e) => setShowRoleNumbers(e.target.checked)} className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
              </div>
            </div>
            ...
            {(() => {
              ...
              const handleTogglePanel = async (faceIndex: number) => {
                ...
                if (newFacePanels[faceIndex]) {
                  ...
                  const panelToRemove = shapes.find(s =>
                    s.type === 'panel' && s.parameters?.parentShapeId === selectedShape.id && s.parameters?.faceIndex === faceIndex
                  );
                  if (panelToRemove) { const { deleteShape } = useAppStore.getState(); deleteShape(panelToRemove.id); }
                } else {
                ...
                if (selectedProfile !== 'none') {
                  setResolving(true);
                  try { await resolveAllPanelJoints(selectedShape.id, selectedProfile); }
                  finally { setResolving(false); }
                }
              };
              ...
              const createVirtualPanel = async (vfId: string, vfIndex: number) => {
                ...
                try {
                  ...
                  const size = new THREE.Vector3();
                  bbox.getSize(size);
                  const axesBySize = [{ index: 0, value: size.x }, { index: 1, value: size.y }, { index: 2, value: size.z }].sort((a, b) => a.value - b.value);
                  ...
                  const newPanel: any = {
                    ...
                    color: '#ffffff',
                    parameters: {
                      width: sizeByIndex[defaultAxis], height: sizeByIndex[altAxis], depth: panelThickness,
                      parentShapeId: selectedShape.id, faceIndex: -(vfIndex + 1),
                      faceRole: vf.role, virtualFaceId: vf.id, arrowRotated: false
                    }
                  };
                  addShape(newPanel);
                  updateVirtualFace(vf.id, { hasPanel: true });
              ...
              const removeVirtualPanel = (vfId: string, vfIndex: number) => {
                const panelToRemove = shapes.find(s =>
                  s.type === 'panel' && s.parameters?.parentShapeId === selectedShape.id && s.parameters?.virtualFaceId === vfId
                );
                if (panelToRemove) { const { deleteShape } = useAppStore.getState(); deleteShape(panelToRemove.id); }
                ...
                <div className={`space-y-0.5 pt-2 border-t border-stone-300 ${isDisabled ? 'opacity-40 pointer-events-none' : ''}`}>
                    ...
                    return (
                      <React.Fragment key={`face-${i}`}>
                        <div
                          ...
                          onClick={(e) => { e.stopPropagation(); if (facePanels[i]) handleRowClick(i); }}
                        >
                          <input
                            type="radio" name="panel-selection" checked={isRowSelected}
                            disabled={isDisabled || !facePanels[i]}
                            onChange={(e) => { e.stopPropagation(); handleRowClick(i); }}
                          ...
                          />
                          <input
                            type="text" value={i + 1} readOnly tabIndex={-1} disabled={isDisabled}
                          ...
                          />
                          <select
                            value={faceRoles[i] || ''} disabled={isDisabled}
                            onClick={(e) => e.stopPropagation()}
                            onChange={async (e) => {
                              ...
                              const panelShape = shapes.find(s =>
                                s.type === 'panel' && s.parameters?.parentShapeId === selectedShape.id &&
                                s.parameters?.faceIndex === i && !s.parameters?.extraRowId
                              );
                              if (panelShape) {
                                updateShape(panelShape.id, { parameters: { ...panelShape.parameters, faceRole: newRole } });
                                if (selectedProfile !== 'none') {
                                  setResolving(true);
                                  try { await rebuildAllPanels(selectedShape.id); await resolveAllPanelJoints(selectedShape.id, selectedProfile); }
                                  finally { setResolving(false); }
                                }
                              }
                            }}
                          ...
                          />
                          <input type="text" value={dimensions?.primary || 'NaN'} readOnly tabIndex={-1} onClick={(e) => e.stopPropagation()} className="w-[48px] px-1 py-0.5 text-xs font-mono border rounded text-center bg-orange-50 text-gray-800 border-orange-300 font-semibold" title="Arrow Direction Dimension" />
                          <input type="text" value={dimensions?.secondary || 'NaN'} readOnly tabIndex={-1} onClick={(e) => e.stopPropagation()} className="w-[48px] px-1 py-0.5 text-xs font-mono border rounded text-center bg-blue-50 text-gray-800 border-blue-300 font-semibold" title="Perpendicular to Arrow Direction" />
                          <input type="text" value={dimensions?.thickness || 'NaN'} readOnly tabIndex={-1} onClick={(e) => e.stopPropagation()} className="w-[48px] px-1 py-0.5 text-xs font-mono border rounded text-center bg-green-50 text-gray-800 border-green-300 font-semibold" title="Panel Thickness" />
                          <input
                            type="checkbox" checked={facePanels[i] || false} disabled={isDisabled}
                            onClick={(e) => e.stopPropagation()} onChange={() => handleTogglePanel(i)}
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              if (panelShape) {
                                ...
                                updateShape(panelShape.id, { parameters: { ...panelShape.parameters, arrowRotated: !current } });
                              }
                            }}
                            className={`p-0.5 rounded transition-colors ${
                              isDisabled || !facePanels[i] ? 'text-stone-300 cursor-not-allowed' : (() => {
                                const ps = shapes.find(s =>
                                  s.type === 'panel' && s.parameters?.parentShapeId === selectedShape.id &&
                                  s.parameters?.faceIndex === i && !s.parameters?.extraRowId
                                );
                                return ps?.parameters?.arrowRotated ? 'text-blue-600 bg-blue-50 hover:bg-blue-100' : 'text-slate-500 hover:bg-stone-100';
                              })()
                            }`}
                  ...
                  {shapeVirtualFaces.map((vf, vfIdx) => {
                    const virtualPanel = shapes.find(s =>
                      s.type === 'panel' && s.parameters?.parentShapeId === selectedShape.id && s.parameters?.virtualFaceId === vf.id
                    );
                    ...
                    const handleVirtualRowClick = () => { if (!vf.hasPanel) return; setSelectedPanelRow(`vf-${vf.id}`); };
                    ...
                    return (
                      <div
                        ...
                        onClick={(e) => { e.stopPropagation(); if (vf.hasPanel) handleVirtualRowClick(); }}
                      >
                        <input
                          type="radio" name="panel-selection" checked={isRowSelected}
                          disabled={isDisabled || !vf.hasPanel}
                          onChange={(e) => { e.stopPropagation(); handleVirtualRowClick(); }}
                        ...
                        <input
                          type="text" value={vf.description || ''} disabled={isDisabled}
                          onClick={(e) => e.stopPropagation()}
                        ...
                        />
                        <input type="text" value={vf.hasPanel ? (arrowRotated ? Math.round(panelHeight) : Math.round(panelWidth)) : '—'} readOnly tabIndex={-1} className={`w-[48px] px-1 py-0.5 text-xs font-mono border rounded text-center ${vf.hasPanel ? 'bg-orange-50 text-gray-800 border-orange-300 font-semibold' : 'bg-orange-50 text-stone-400 border-orange-200'}`} title="Arrow Direction Dimension" onClick={(e) => e.stopPropagation()} />
                        <input type="text" value={vf.hasPanel ? (arrowRotated ? Math.round(panelWidth) : Math.round(panelHeight)) : '—'} readOnly tabIndex={-1} className={`w-[48px] px-1 py-0.5 text-xs font-mono border rounded text-center ${vf.hasPanel ? 'bg-blue-50 text-gray-800 border-blue-300 font-semibold' : 'bg-blue-50 text-stone-400 border-blue-200'}`} title="Perpendicular Dimension" onClick={(e) => e.stopPropagation()} />
                        <input type="text" value={vf.hasPanel ? Math.round(panelDepth) : '—'} readOnly tabIndex={-1} className={`w-[48px] px-1 py-0.5 text-xs font-mono border rounded text-center ${vf.hasPanel ? 'bg-green-50 text-gray-800 border-green-300 font-semibold' : 'bg-green-50 text-stone-400 border-green-200'}`} title="Panel Thickness" onClick={(e) => e.stopPropagation()} />
                        <input
                          type="checkbox" checked={vf.hasPanel} disabled={isDisabled}
                          onClick={(e) => e.stopPropagation()}
                          onChange={async () => {
                            if (vf.hasPanel) {
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            if (virtualPanel) {
                              ...
                              updateShape(virtualPanel.id, { parameters: { ...virtualPanel.parameters, arrowRotated: !current } });
                            }
                          }}
                          className={`p-0.5 rounded transition-colors ${isDisabled || !vf.hasPanel ? 'text-stone-300 cursor-not-allowed' : arrowRotated ? 'text-blue-600 bg-blue-50 hover:bg-blue-100' : 'text-slate-500 hover:bg-stone-100'}`}
                          title="Rotate arrow direction"
                        >
                          <RotateCw size={13} />
                        </button>
