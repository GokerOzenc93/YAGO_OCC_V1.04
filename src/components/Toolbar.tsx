...
const Toolbar: React.FC<ToolbarProps> = ({ onOpenCatalog }) => {
  ...
  const handleAddBox = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      addShape({
        id: `box-${Date.now()}`, type: 'box',
        geometry: convertReplicadToThreeGeometry(replicadShape),
        replicadShape,
        position: [0, 0, 0], rotation: [0, 0, 0], scale: [1, 1, 1],
        color: '#2563eb', parameters: { width: w, height: h, depth: d }
      });
    } catch (error) {
    ...
    try {
      ...
      for (const target of intersecting) {
        ...
        updateShape(target.id, {
          geometry: newGeo, replicadShape: result,
          subtractionGeometries: [...(target.subtractionGeometries || []), { geometry: sel.geometry.clone(), relativeOffset: relOff, relativeRotation: relRot, scale: [1, 1, 1] }],
          parameters: { ...target.parameters, scaledBaseVertices: newVerts.map(v => [v.x, v.y, v.z]) }
        });
      }
    ...
    <div className="flex flex-col font-inter">
        ...
        <div className="flex items-center h-full">
          {menus.map((menu) => (
            <div key={menu.label} className="relative h-full">
              <button
                className={`h-full px-3 text-xs font-medium hover:bg-stone-100 transition-colors flex items-center ${activeMenu === menu.label ? 'bg-stone-100 text-slate-800' : 'text-slate-700'}`}
      ...
      <div className="flex items-center h-12 gap-3 px-4 bg-stone-50 border-b border-stone-200">
        <div className="flex items-center gap-0.5 bg-white rounded-lg p-1 shadow-sm border border-stone-200">
          {[{ icon: <FilePlus size={11} />, label: 'New', shortcut: 'Ctrl+N' }, { icon: <Save size={11} />, label: 'Save', shortcut: 'Ctrl+S' }, { icon: <FileDown size={11} />, label: 'Save As', shortcut: 'Ctrl+Shift+S' }].map((btn, i) => (
            <button key={i} className="p-1.5 rounded text-stone-600 hover:bg-stone-50 hover:text-slate-800 transition-colors" title={`${btn.label} (${btn.shortcut})`}>{btn.icon}</button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-0.5 bg-white rounded-lg p-1 shadow-sm border border-stone-200">
          {[{ icon: <Undo2 size={11} />, label: 'Undo', shortcut: 'Ctrl+Z' }, { icon: <Redo2 size={11} />, label: 'Redo', shortcut: 'Ctrl+Y' }].map((btn, i) => (
            <button key={i} className="p-1.5 rounded text-stone-600 hover:bg-stone-50 hover:text-slate-800 transition-colors" title={`${btn.label} (${btn.shortcut})`}>{btn.icon}</button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-0.5 bg-white rounded-lg p-1 shadow-sm border border-stone-200">
          <button className={`p-1.5 rounded transition-all ${activeTool === Tool.DIMENSION ? 'bg-orange-100 text-orange-700 border border-orange-300' : 'hover:bg-stone-50 text-stone-600 hover:text-slate-800'}`}
            onClick={() => setActiveTool(activeTool === Tool.DIMENSION ? Tool.SELECT : Tool.DIMENSION)} title="Dimension (D)">
            <Ruler size={11} />
          </button>
        </div>
