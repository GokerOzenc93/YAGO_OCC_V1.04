...
export function createPanelFromFaceGroup(
...
): ExtrudedPanelInfo | null {
  if (groupIndex < 0 || groupIndex >= faceGroups.length) return null;

  const group = faceGroups[groupIndex];
  const allFaceIndices: number[] = group.faceIndices;

  if (allFaceIndices.length === 0) return null;

  const allVertices: THREE.Vector3[] = [];
  allFaceIndices.forEach(idx => {
    const face = faces[idx];
    if (face) {
  ...
  const rightShrink = adjustments?.widthShrink?.right || 0;
  ...
  const extrudedGeometry = extrudeFaceGeometry(faceGeometry, avgNormal, panelThickness, true);
  faceGeometry.dispose();

  const box = new THREE.Box3().setFromBufferAttribute(
    extrudedGeometry.getAttribute('position') as THREE.BufferAttribute
  );
  const center = new THREE.Vector3();
