...
export async function subtractRaycastPanelsFromParent(parentShapeId: string): Promise<void> {
  ...
  if (raycastPanels.length === 0) return;

  console.log(`Subtracting ${raycastPanels.length} raycast panel(s) from parent ${parentShapeId}...`);

  const { createReplicadBox, performBooleanCut, convertReplicadToThreeGeometry, initReplicad } = await import('./ReplicadService');
  const { getOriginalSize, applyFillets, updateFilletCentersForNewGeometry } = await import('./ShapeUpdaterService');
  const { getReplicadVertices } = await import('./VertexEditorService');

  await initReplicad();

  const parentPos = new THREE.Vector3(parentShape.position[0], parentShape.position[1], parentShape.position[2]);
  const parentRot = parentShape.rotation as [number, number, number];
  const invRot = new THREE.Quaternion()
    .setFromEuler(new THREE.Euler(parentRot[0], parentRot[1], parentRot[2], 'XYZ'))
    .invert();

  const parentGeoBox = new THREE.Box3().setFromBufferAttribute(
    parentShape.geometry.attributes.position as THREE.BufferAttribute
  );
  const parentGeoCenter = new THREE.Vector3();
  const parentGeoSize = new THREE.Vector3();
  parentGeoBox.getCenter(parentGeoCenter);
  parentGeoBox.getSize(parentGeoSize);

  const isCentered =
    Math.abs(parentGeoCenter.x) < 0.01 &&
    Math.abs(parentGeoCenter.y) < 0.01 &&
    Math.abs(parentGeoCenter.z) < 0.01;

  const parentCornerWorld = parentPos.clone();
  if (isCentered) {
    const halfLocal = new THREE.Vector3(parentGeoSize.x / 2, parentGeoSize.y / 2, parentGeoSize.z / 2);
    halfLocal.applyQuaternion(new THREE.Quaternion().setFromEuler(new THREE.Euler(parentRot[0], parentRot[1], parentRot[2], 'XYZ')));
    parentCornerWorld.sub(halfLocal);
  }
  ...
  for (const sub of existingSubs) {
    try {
      const subSize = getOriginalSize(sub.geometry);
      const subBox = await createReplicadBox({ width: subSize.x, height: subSize.y, depth: subSize.z });
      currentReplicadShape = await performBooleanCut(
        currentReplicadShape, subBox,
  ...
  for (const panel of raycastPanels) {
    try {
      const panelGeoBox = new THREE.Box3().setFromBufferAttribute(
        panel.geometry.attributes.position as THREE.BufferAttribute
      );
      const panelGeoCenter = new THREE.Vector3();
      const panelGeoSize = new THREE.Vector3();
      ...
      const panelCornerLocal = new THREE.Vector3(
        ...
        panelLocalFromParentCorner.y - panelGeoSize.y / 2,
        panelLocalFromParentCorner.z - panelGeoSize.z / 2
      );

      const cutBox = await createReplicadBox({
        width: panelGeoSize.x,
        height: panelGeoSize.y,
        depth: panelGeoSize.z,
      });

      const relOffset: [number, number, number] = [
        panelCornerLocal.x,
        panelCornerLocal.y,
        panelCornerLocal.z,
      ];
      ...
      newSubtractions.push({
        ...
        parameters: {
          ...
          depth: String(panelGeoSize.z),
          posX: String(relOffset[0]),
          posY: String(relOffset[1]),
          posZ: String(relOffset[2]),
          rotX: '0',
          rotY: '0',
          rotZ: '0',
        },
      ...
      console.warn(`subtractRaycastPanelsFromParent: cut failed for panel ${panel.id}:`, panelErr);
    }
  }

  let updatedFillets = parentShape.fillets || [];
  let newGeometry = convertReplicadToThreeGeometry(currentReplicadShape);

  if (updatedFillets.length > 0) {
    updatedFillets = await updateFilletCentersForNewGeometry(updatedFillets, newGeometry, { width: baseW, height: baseH, depth: baseD });
    currentReplicadShape = await applyFillets(currentReplicadShape, updatedFillets, { width: baseW, height: baseH, depth: baseD });
    newGeometry = convertReplicadToThreeGeometry(currentReplicadShape);
  }

  const newBaseVertices = await getReplicadVertices(currentReplicadShape);
  ...
  useAppStore.getState().updateShape(parentShapeId, {
    ...
    parameters: {
      ...parentShape.parameters,
      scaledBaseVertices: newBaseVertices.map(v => [v.x, v.y, v.z]),
    },
  });

  console.log(`subtractRaycastPanelsFromParent: ${raycastPanels.length} panel(s) subtracted from ${parentShapeId}`);
}
