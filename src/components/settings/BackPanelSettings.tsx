...
export function BackPanelSettings({
  ...
  const checkCollisions = () => {
    ...
    const leftPanelDepth = cabinetDepth - (leftPanelBackShorten / 1000);
    const rightPanelDepth = cabinetDepth - (rightPanelBackShorten / 1000);
    const topPanelDepth = cabinetDepth - (topPanelBackShorten / 1000);
    const bottomPanelDepth = cabinetDepth - (bottomPanelBackShorten / 1000);

    const backPanelZ = -cabinetDepth / 2 + (grooveOffset / 1000) + (backPanelThickness / 1000) / 2;
    const backPanelFrontEdge = backPanelZ - (backPanelThickness / 1000) / 2;

    const leftPanelBackEdge = cabinetDepth / 2 - leftPanelDepth;
    const rightPanelBackEdge = cabinetDepth / 2 - rightPanelDepth;
    const topPanelBackEdge = cabinetDepth / 2 - topPanelDepth;
    const bottomPanelBackEdge = cabinetDepth / 2 - bottomPanelDepth;

    let needsUpdate = false;
    let newLeftExtend = backPanelLeftExtend;
    let newRightExtend = backPanelRightExtend;
    let newTopExtend = backPanelTopExtend;
    let newBottomExtend = backPanelBottomExtend;

    if (backPanelFrontEdge < leftPanelBackEdge) {
      const penetration = leftPanelBackEdge - backPanelFrontEdge;
      const maxAllowedExtend = Math.max(0, (grooveDepth - penetration) * 1000);
      if (backPanelLeftExtend > maxAllowedExtend) {
        newLeftExtend = maxAllowedExtend;
        needsUpdate = true;
        console.log('🔴 Collision detected: Back panel penetrating left panel. Reducing left extend to:', maxAllowedExtend.toFixed(1));
      }
    }

    if (backPanelFrontEdge < rightPanelBackEdge) {
      const penetration = rightPanelBackEdge - backPanelFrontEdge;
      const maxAllowedExtend = Math.max(0, (grooveDepth - penetration) * 1000);
      if (backPanelRightExtend > maxAllowedExtend) {
        newRightExtend = maxAllowedExtend;
        needsUpdate = true;
        console.log('🔴 Collision detected: Back panel penetrating right panel. Reducing right extend to:', maxAllowedExtend.toFixed(1));
      }
    }

    if (backPanelFrontEdge < topPanelBackEdge) {
      const penetration = topPanelBackEdge - backPanelFrontEdge;
      const maxAllowedExtend = Math.max(0, (grooveDepth - penetration) * 1000);
      if (backPanelTopExtend > maxAllowedExtend) {
        newTopExtend = maxAllowedExtend;
        needsUpdate = true;
        console.log('🔴 Collision detected: Back panel penetrating top panel. Reducing top extend to:', maxAllowedExtend.toFixed(1));
      }
    }

    if (backPanelFrontEdge < bottomPanelBackEdge) {
      const penetration = bottomPanelBackEdge - backPanelFrontEdge;
      const maxAllowedExtend = Math.max(0, (grooveDepth - penetration) * 1000);
      if (backPanelBottomExtend > maxAllowedExtend) {
        newBottomExtend = maxAllowedExtend;
        needsUpdate = true;
        console.log('🔴 Collision detected: Back panel penetrating bottom panel. Reducing bottom extend to:', maxAllowedExtend.toFixed(1));
      }
    }

    if (needsUpdate) {
      if (newLeftExtend !== backPanelLeftExtend) {
        setBackPanelLeftExtend(newLeftExtend);
      }
      if (newRightExtend !== backPanelRightExtend) {
        setBackPanelRightExtend(newRightExtend);
      }
      if (newTopExtend !== backPanelTopExtend) {
        setBackPanelTopExtend(newTopExtend);
      }
      if (newBottomExtend !== backPanelBottomExtend) {
        setBackPanelBottomExtend(newBottomExtend);
      }
    }
  };
