...
export const FaceRaycastOverlay: React.FC<FaceRaycastOverlayProps> = ({ shape, allShapes = [] }) => {
  const { raycastMode, setRaycastMode, addShape, selectedShapeId } = useAppStore();
  ...
  const handleCreatePanel = useCallback(async () => {
    ...
    try {
      ...
      const localNormal = normal.clone().applyQuaternion(invRot).normalize();
      ...
      const newPanel: any = {
        ...
        parameters: {
          ...
          raycastFaceNormal: [localNormal.x, localNormal.y, localNormal.z] as [number, number, number],
