...
function collectBoundaryEdgesWorld(
  faces: FaceData[], faceIndices: number[], localToWorld: THREE.Matrix4
...
function collectPanelObstacleEdgesWorld(
  panelShapes: any[], facePlaneNormal: THREE.Vector3, facePlaneOrigin: THREE.Vector3, planeTolerance: number = 15
): Array<{ v1: THREE.Vector3; v2: THREE.Vector3 }> {
...
function collectSubtractionObstacleEdgesWorld(
  subtractions: any[], parentLocalToWorld: THREE.Matrix4,
  facePlaneNormal: THREE.Vector3, facePlaneOrigin: THREE.Vector3, planeTolerance: number = 20
): Array<{ v1: THREE.Vector3; v2: THREE.Vector3 }> {
...
export function getSubtractorFootprints2D(
  subtractions: any[], parentLocalToWorld: THREE.Matrix4,
  facePlaneNormal: THREE.Vector3, facePlaneOrigin: THREE.Vector3,
  u: THREE.Vector3, v: THREE.Vector3, planeTolerance: number = 50
): Point2D[][] {
  ...
  for (const sub of subtractions) {
    ...
    for (let i = 0; i < posAttr.count; i++) {
      ...
      if (dist < planeTolerance) onPlaneVerts.push(wp);
    }
...
function traceHoleEdge(entryPoint: Point2D, hole: Point2D[], subject: Point2D[]): Point2D[] {
  ...
  for (let i = 0; i < subject.length; i++) subjectEdges.push([subject[i], subject[(i + 1) % subject.length]]);
...
export function subtractPolygon(subject: Point2D[], hole: Point2D[]): Point2D[] {
  ...
  for (let i = 0; i < hole.length; i++) holeEdges.push([hole[i], hole[(i + 1) % hole.length]]);
  ...
  for (let i = 0; i < subject.length; i++) {
    ...
    for (const inter of intersections) {
      ...
      for (const hp of holeTraversal) result.push(hp);
    }
  }
...
function castRayOnFaceWorld(
  originWorld: THREE.Vector3, dirWorld: THREE.Vector3,
...
function collectVirtualFaceObstacleEdgesWorld(
  virtualFaces: VirtualFace[], excludeId: string | null, shapeLocalToWorld: THREE.Matrix4,
  facePlaneNormal: THREE.Vector3, facePlaneOrigin: THREE.Vector3, planeTolerance: number = 20
): Array<{ v1: THREE.Vector3; v2: THREE.Vector3 }> {
  const edges: Array<{ v1: THREE.Vector3; v2: THREE.Vector3 }> = [];
...
function buildPreview(
  ...
  localToWorld: THREE.Matrix4, worldToLocal: THREE.Matrix4, childPanels: any[],
  shapeId: string, subtractions: any[] = [], geometry?: THREE.BufferGeometry,
  ...
  for (const dir of directions) {
    ...
    lines.push({ start: startWorld.clone().sub(parentPos), end: hitWorld.clone().sub(parentPos) });
    hitPointsWorld.push(hitWorld);
  }
  if (hitPointsWorld.length < 4) return null;
  const uPosHit = hitPointsWorld[0], uNegHit = hitPointsWorld[1], vPosHit = hitPointsWorld[2], vNegHit = hitPointsWorld[3];
  ...
  let rect2D: Point2D[] = ensureCCW([
    { x: uPosT, y: vPosT }, { x: -uNegT, y: vPosT },
    { x: -uNegT, y: -vNegT }, { x: uPosT, y: -vNegT },
  ]);
...
export const FaceRaycastOverlay: React.FC<FaceRaycastOverlayProps> = ({ shape, allShapes = [] }) => {
  ...
  useEffect(() => {
    if (!raycastMode) { setHoveredGroupIndex(null); setPending(null); }
  }, [raycastMode]);
  ...
  const handlePointerDown = (e: any) => {
    ...
    const preview = buildPreview(
      clickWorld, group, faces, localToWorld, worldToLocal, childPanels,
      shape.id, shape.subtractionGeometries || [], shape.geometry, shapeVirtualFaces
    );
    setPending(preview);
  };
    ...
    <>
      ...
      {hoverHighlightGeometry && !pending && (
        <mesh geometry={hoverHighlightGeometry}>
          <meshBasicMaterial
            color={0xfbbf24} transparent opacity={0.35} side={THREE.DoubleSide}
      ...
      {pending && (
        <>
          ...
          <mesh geometry={pending.geo}>
            <meshBasicMaterial
              color={0x22c55e} transparent opacity={0.5} side={THREE.DoubleSide}
